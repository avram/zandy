 ...
 ******************************************************************************/
package com.mattrobertson.zotable.app.task;
...
import com.mattrobertson.zotable.app.ServerCredentials;
import com.mattrobertson.zotable.app.XMLResponseParser;
import com.mattrobertson.zotable.app.data.Attachment;
...
public class ZoteroAPITask extends AsyncTask<APIRequest, Message, Message> {
	private static final String TAG = "ZoteroAPITask";
	...
	{	
        ...
        for (int i = 0; i < count; i++) {
        	...
        	// The XML parser's buildDirtyQueue is simply from following continuations in the paged
	    	...
	    	if (XMLResponseParser.queue != null && !XMLResponseParser.queue.isEmpty()) {
	        	Log.i(TAG, "Finished call, but adding " +
	        				...
	        				" items to buildDirtyQueue.");
	    		queue.addAll(XMLResponseParser.queue); 
	    		XMLResponseParser.queue.clear();
	    	} else {
	        	Log.i(TAG, "Finished call, and parser's request buildDirtyQueue is empty");
	    	}
        }
        ...
        if (queue.size() > 0) {
        	...
        	if (false && !XMLResponseParser.followNext) {
        		...
        		for (APIRequest r : queue) {
        			if (r.type != APIRequest.ITEMS_CHILDREN) {
        				Log.d(TAG, "Removing request from buildDirtyQueue since last page had old items: "+r.query);
        				toRemove.add(r);
        			}
        		}
        	...
        	// case, we could just save our buildDirtyQueue here and really return.
        	
        	// XXX Test: Here, we try to use doInBackground instead
    		doInBackground(requests);
    		
    		// Return a message with the number of requests added to the buildDirtyQueue
        	Message msg = Message.obtain();
    	...
    	// This buildDirtyQueue has deletions, collection memberships, and failing requests
