...
public class ItemCollection extends HashSet<Item> {
	...
	public boolean add(Item item, boolean fromAPI, Database db) {
		...
		super.add(item);
		Log.d(TAG, "Item added to collection: " + item.getTitle() + " (" + getTitle() + ")");
		if (!fromAPI) {
		...
		if (existing == null) {
			try {
				SQLiteStatement insert = db.compileStatement("insert or replace into collections " +
					"(collection_name, collection_key, collection_parent, etag, dirty, collection_size, timestamp, fav)" +
					" values (?, ?, ?, ?, ?, ?, ?, ?)");
				...
				if (parentKey == null)
					...
					insert.bindString(3, parentKey);

				if (etag == null)
					insert.bindNull(4);
				else
					insert.bindString(4, etag);

				if (dirty == null)
					insert.bindNull(5);
				else
					insert.bindString(5, dirty);
				...
				if (timestamp == null)
					...
					insert.bindString(7, timestamp);

				insert.bindLong(8, isFav ? 1 : 0);
			...
			try {
				SQLiteStatement update = db.compileStatement("update collections set " +
						"collection_name=?, etag=?, dirty=?, collection_size=?, timestamp=?, fav=?" +
						" where _id=?");
				update.bindString(1, title);
				if (etag == null)
					update.bindNull(2);
				else
					update.bindString(2, etag);

				if (dirty == null)
					update.bindNull(3);
				else
					update.bindString(3, dirty);
				...
				if (timestamp == null)
					...
					update.bindString(5, timestamp);

				update.bindLong(6, isFav ? 1 : 0);

				update.bindString(7, dbId);
	...
	public static ItemCollection load(Cursor cur) {
		...
		coll.isFav = cur.getLong(8) == 1 ? true : false;
	...
	public static void queue(Database db) {
		Log.d(TAG,"Clearing dirty buildDirtyQueue before repopulation");
		queue.clear();
		ItemCollection coll;
		String[] cols = Database.COLLCOLS;
		...
		do {
			Log.d(TAG,"Adding collection to dirty buildDirtyQueue");
			coll = load(cur);
			queue.add(coll);
		} while (cur.moveToNext() != false);
	...
	public static ArrayList<ItemCollection> getCollections(Item i, Database db) {
		...
		Cursor cursor = db.rawQuery("SELECT collection_name, collection_parent," +
				...
				" timestamp, fav FROM collections, itemtocollections" +
