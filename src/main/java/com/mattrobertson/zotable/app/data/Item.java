...
public class Item {
	 ...
	 */
	public static ArrayList<Item> dirtyQueue = new ArrayList<Item>();
	...
	public void setDate(String date) {
		...
		try {
			...
			Log.e(TAG, "Exception setting date", e);
		}
	}
	...
	public void setContent(JSONObject content) {
		if (!this.content.toString().equals(content.toString())) {	// not identical -- some update to the item has occurred
			if (!APIRequest.API_CLEAN.equals(this.dirty))
				this.dirty = APIRequest.API_DIRTY;
	...
	public ArrayList<Bundle> tagsToBundleArray() {
		...
		try {
			...
			for (int i = 0; i < tags.length(); i++) {
				b = new Bundle();
				// Type is not always specified, but we try to get it and fall back to 0 when missing.
    ...
    public boolean hasTag(String tag) {
        ArrayList<Bundle> tags = tagsToBundleArray();
    ...
    public void addTag(String tag) {
        ...
        try {
            // If there is no tags array for some reason, create one
            if (!itemContent.has("tags"))
                itemContent.put("tags", new JSONArray());

            JSONArray jsoTags = itemContent.getJSONArray("tags");

            JSONObject objTag = new JSONObject();
            objTag.put("tag",tag);
            objTag.put("type","User");  // assuming tag is being added by user (as opposed to "Auto")
            objTag.put("itemKey",this.key);
    ...
    public void removeTag(String tag) {
        ...
        try {
            ...
            for (int i=0; i < jsoTags.length(); i++) {
                if (jsoTags.getJSONObject(i).get("tag").equals(tag)) {
                    jsoTags.remove(i);
                    return;
                }
            }
	...
	public void save(Database db) {
		...
		if (dbId == null && existing == null) {	// Does Not Exist in db
			...
			if (cur != null)
				cur.close();
			Item itemFromDb = load(key, db);
			dbId = itemFromDb.dbId;

			// Parse tags from JSON content & insert into db
			try {
				JSONArray jTagsArr = content.getJSONArray("tags");

				Cursor curTags = null;

				for (int i=0; i<jTagsArr.length(); i++) {
					JSONObject jTag = jTagsArr.getJSONObject(i);
					String tag = jTag.getString("tag");

					String[] argsTags = {dbId, tag};
					curTags = db.rawQuery(
				...
				Log.d(TAG,"JSONException in Item: " + e.getMessage());
			}
		}
	...
	public void delete(Database db) {
        ...
        db.rawQuery("delete from tags where item_id=?", args);
	...
	/**
	 * Identifies dirty (actually, non-clean) items in the database and queues them for syncing
	 */
	public static void buildDirtyQueue(Database db) {
		Log.d(TAG, "Clearing dirty-item queue before repopulation");
		dirtyQueue.clear();
		Item item;
		String[] cols = Database.ITEMCOLS;
		...
		do {
			item = load(cur);
			dirtyQueue.add(item);
			Log.d(TAG, "Adding item to dirty queue: " + item.getTitle());
	...
	public static void setAllClean(Database db) {
		Log.d(TAG, "Setting all items to sync-status=clean");
		dirtyQueue.clear();

		String[] args = {APIRequest.API_CLEAN};

		db.rawQuery("update items set dirty=?", args);
	}
