...
this.LINK_MODE_LINKED_URL = 3;
	...
	public static void queue(Database db) {
		if (queue == null) {
			// Initialize the buildDirtyQueue if necessary
			queue = new ArrayList<Attachment>();
		}
		Log.d(TAG, "Clearing attachment dirty buildDirtyQueue before repopulation");
		queue.clear();
		...
		do {
			Log.d(TAG, "Adding attachment to dirty buildDirtyQueue");
			attachment = load(cur);
