...
public class IntObjHashtable<V> implements Cloneable {
     ...
     */
    public IntObjHashtable(int initialCapacity, float loadFactor) {
        super();
        if (initialCapacity < 0) {
            throw new IllegalArgumentException(String.format("Illegal capacity %d", initialCapacity));
        }
        if (loadFactor <= 0) {
            throw new IllegalArgumentException(String.format("Illegal load %s", String.valueOf(loadFactor)));
        }
        if (initialCapacity == 0) {
            initialCapacity = 1;
        }
    ...
    static class Entry<V> {
        ...
        protected Object clone() {
        	Entry<V> entry = new Entry(hash, key, value, next != null ? (Entry<V>)next.clone() : null);
        	return entry;
        }
    }

    // extra inner class by Paulo
    static class IntHashtableIterator<V> implements Iterator<Entry<V>> {
        int index;
        Entry<V> table[];
        Entry<V> entry;

        IntHashtableIterator(Entry<V> table[]) {
        	this.table = table;
...
// extra methods by Paulo Soares:
    ...
    public Object clone() {
    	try {
    		IntObjHashtable t = (IntObjHashtable)super.clone();
